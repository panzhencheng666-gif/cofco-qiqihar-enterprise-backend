...
assert_contains "$deploy_script" 'migration_tls_mount_args+=(--volume "${decoded_path}:${decoded_path}:ro")'
...
assert_contains "$deploy_script" 'install -o 10001 -g 10001 -m 400 "$decoded_path" "${tls_root}/${parameter_key}"'
