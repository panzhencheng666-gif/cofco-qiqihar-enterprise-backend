...
assert_contains "$deploy_script" '[[ "$decoded_path" == /run/* ]]'
assert_contains "$deploy_script" 'staged_path="${runtime_run_root}/${decoded_path#/run/}"'
assert_contains "$deploy_script" 'migration_tls_mount_args+=(--volume "${runtime_run_root}:/run:ro")'
...
assert_contains "$unit" '/var/lib/cofco/risk-intelligence/runtime-run:/run:ro'
