...
- 样本表触发器使后续新样本、移入已分配地区的样本继承负责人。新增及导入不要求提交维护人，由已有地区责任查询决定；未配置责任时保持未分配，不默认认领给操作人。旧模板维护人员工账号列可留空，若提交则必须与地区负责人一致。
- 普通样本更新省略维护人时保留当前维护人，显式提交其他人一律返回 403 FORMAL_SAMPLE_RESPONSIBILITY_ACCOUNT_ONLY，管理员也不例外。旧单点维护人接口先认证/授权，再返回同一拒绝码，既不写样本也不记改派成功事件。改派仅通过账号负责地区的完整预览、确认、保存流程。原实际填报人和历史观测不改写。
