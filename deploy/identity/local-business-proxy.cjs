...
https.createServer(tls,(req,res)=>{
 if(req.url==='/first-admin/complete'){
  if(req.method!=='POST'||req.headers.origin!=='https://localhost:29443'){res.writeHead(403);return res.end()}
  if(!fs.existsSync(__dirname+'/first-admin-token')){res.writeHead(410);return res.end()}
  const body=JSON.stringify({token:fs.readFileSync(__dirname+'/first-admin-token','utf8')});
  const headers={'Content-Type':'application/json','Content-Length':Buffer.byteLength(body),'Cookie':req.headers.cookie||'','X-XSRF-TOKEN':req.headers['x-xsrf-token']||'','X-Forwarded-Proto':'https','X-Forwarded-Host':'localhost:29443'};
  const p=http.request({host:'127.0.0.1',port:28090,path:'/api/v1/identity/invitations/first-administrator',method:'POST',headers},r=>{if(r.statusCode>=200&&r.statusCode<300)fs.unlinkSync(__dirname+'/first-admin-token');res.writeHead(r.statusCode,r.headers);r.pipe(res)});p.on('error',()=>{res.writeHead(502);res.end()});p.end(body);return;
 }
 if(req.url==='/activation-required.html'){res.setHeader('Content-Type','text/html; charset=utf-8');res.setHeader('Cache-Control','no-store');return res.end(fs.readFileSync(__dirname+'/activation-required.html'))}
 ...
 if(req.url==='/first-admin.html'){res.setHeader('Content-Type','text/html; charset=utf-8');res.setHeader('Cache-Control','no-store');return res.end(fs.readFileSync(__dirname+'/first-admin.html'))}
 ...
 let f=path.resolve(dist,'.'+rel);if(!f.startsWith(dist+'/'))f=dist+'/index.html';if(!fs.existsSync(f)||fs.statSync(f).isDirectory())f=dist+'/index.html';
