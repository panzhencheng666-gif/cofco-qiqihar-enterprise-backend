-- Canonical master data stays in its owning relations. Governance is append-only:
-- an immutable request is independently reviewed, reaches an explicit effective
-- boundary and is then applied through one controlled database entry point.
CREATE TABLE platform.master_data_revision (
    revision_id bigserial PRIMARY KEY,
    entity_type varchar(30) NOT NULL
        CHECK (entity_type IN ('SUBJECT','REGION','PRODUCT','OBJECT_TYPE')),
    entity_key varchar(240) NOT NULL,
    revision_no integer NOT NULL CHECK (revision_no > 0),
    operation_code varchar(20) NOT NULL CHECK (operation_code IN ('BASELINE','INSERT','UPDATE','DELETE')),
    source_relation varchar(160) NOT NULL,
    source_key varchar(240) NOT NULL,
    governance_state varchar(20) NOT NULL CHECK (governance_state IN ('ACTIVE','RETIRED')),
    snapshot jsonb NOT NULL,
    changed_at timestamptz NOT NULL DEFAULT now(),
    changed_by varchar(120) NOT NULL,
    UNIQUE (entity_type,entity_key,revision_no)
);

COMMENT ON TABLE platform.master_data_revision IS
    'Append-only source lineage and version snapshots for canonical subjects, regions, products and object types.';
COMMENT ON COLUMN platform.master_data_revision.entity_key IS
    'Stable business key. Subjects use business-domain:subject-id and never a name, contact or coordinate.';
COMMENT ON COLUMN platform.master_data_revision.source_relation IS
    'Authoritative owning database relation; local or inferred substitutes are not valid formal sources.';

INSERT INTO platform.master_data_revision(
  entity_type,entity_key,revision_no,operation_code,source_relation,source_key,
  governance_state,snapshot,changed_by)
SELECT 'REGION',code,1,'BASELINE','platform.region',code,'ACTIVE',to_jsonb(region),'flyway-v107'
FROM platform.region region
UNION ALL
SELECT 'PRODUCT',code,1,'BASELINE','platform.product',code,'ACTIVE',to_jsonb(product),'flyway-v107'
FROM platform.product product
UNION ALL
SELECT 'OBJECT_TYPE',code,1,'BASELINE','platform.object_type',code,'ACTIVE',to_jsonb(object_type),'flyway-v107'
FROM platform.object_type object_type
UNION ALL
SELECT 'SUBJECT',business_domain || ':' || subject_id,1,'BASELINE',
       'registry.sample_point_subject_identity',sample_point_id::text,'ACTIVE',
       to_jsonb(subject_identity),'flyway-v107'
FROM registry.sample_point_subject_identity subject_identity;

CREATE TABLE platform.master_data_change_request (
...
CREATE TABLE platform.master_data_change_event (
...
CREATE INDEX master_data_change_event_request_order
...
ALTER TABLE platform.master_data_revision
    ADD COLUMN change_request_id bigint REFERENCES platform.master_data_change_request(request_id),
    ADD COLUMN reviewed_by varchar(120),
    ADD COLUMN review_basis text,
    ADD CONSTRAINT master_data_revision_governance_evidence CHECK (
      (operation_code='BASELINE' AND change_request_id IS NULL AND reviewed_by IS NULL AND review_basis IS NULL)
      OR
...
CREATE FUNCTION platform.reject_append_only_master_data_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE FUNCTION platform.submit_master_data_change(
...
CREATE FUNCTION platform.review_master_data_change(
...
CREATE FUNCTION platform.guard_controlled_master_data_apply()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE FUNCTION platform.capture_master_data_revision()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE FUNCTION platform.apply_master_data_change(applied_request_id bigint,apply_actor varchar)
RETURNS boolean
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE FUNCTION platform.govern_master_data_change(
    requested_entity_type varchar,
    requested_entity_key varchar,
    requested_operation varchar,
    requested_snapshot jsonb,
