...
public class OverviewService {
    ...
    public List<OverviewRegion> regions(
            String parentCode, String productCode, Integer year, String periodCode) {
        ...
        return repository.regions(parentCode, productCode, effectiveYear, scope.regionCodes());
    }

    @Transactional(readOnly = true)
    public List<OverviewRegion> locations(
            String ancestorCode, String level, String productCode, Integer year, String periodCode) {
        if (blank(productCode) || !repository.knownProduct(productCode)
                || (!blank(periodCode) && !repository.knownPeriod(periodCode))
        ...
        return repository.locations(ancestorCode, level, productCode, effectiveYear, scope.regionCodes());
    }

    @Transactional(readOnly = true)
    public List<OverviewIndicator> indicators(
            String productCode, String regionCode, Integer year, String periodCode) {
        if (blank(productCode) || blank(regionCode)
    ...
    private int effectiveYear(Integer year, String periodCode) {
        ...
        if (effective == null && !blank(periodCode)) {
            effective = repository.surveyYearForPeriod(periodCode).orElseThrow(OverviewService::invalid);
        }
        if (effective == null || effective < 1900 || effective > 2200) {
            throw invalid();
        }
        return effective;
    }
