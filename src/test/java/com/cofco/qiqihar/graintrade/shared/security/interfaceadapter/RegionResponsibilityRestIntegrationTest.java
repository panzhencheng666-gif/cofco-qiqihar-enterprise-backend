...
class RegionResponsibilityRestIntegrationTest {
    static final String ADMINISTRATOR="region-responsibility-administrator";
    ...
    @BeforeEach void prepare() {
        ...
        if(subject==null) {
            jdbc.sql("DELETE FROM platform.region_responsibility WHERE region_code=:region")
                .param("region",REGION).update();
            jdbc.sql("DELETE FROM platform.security_user_region_scope WHERE region_code=:region AND subject_id LIKE 'region-responsibility-%'")
                .param("region",REGION).update();
        }
        subject="region-responsibility-"+UUID.randomUUID();
        jdbc.sql("INSERT INTO platform.security_user(subject_id,display_name,work_unit_code,enabled) VALUES(:subject,'地区责任管理员','TEST',true) ON CONFLICT(subject_id) DO UPDATE SET enabled=true,account_status='ACTIVE',employment_status='ACTIVE'")
            .param("subject",ADMINISTRATOR).update();
        jdbc.sql("INSERT INTO platform.security_user_role(subject_id,role_code) VALUES(:subject,'BUSINESS_REVIEWER') ON CONFLICT DO NOTHING")
            .param("subject",ADMINISTRATOR).update();
    ...
    @Test void savesIndependentResponsibilityAndNewSamplesInheritIt() throws Exception {
        ...
        mvc.perform(put(path()).principal(()->ADMINISTRATOR).contentType(MediaType.APPLICATION_JSON)
        ...
        mvc.perform(get("/api/v1/identity/employees/"+subject).principal(()->ADMINISTRATOR))
        ...
        mvc.perform(get(path()).principal(()->ADMINISTRATOR))
            .andExpect(status().isOk()).andExpect(jsonPath("$.data.regionCodes[0]").value(REGION));
    ...
    @Test void rejectsOldSingleSampleReassignmentAndKeepsHistoricalReporter() throws Exception {
        ...
        mvc.perform(put("/api/v1/formal-sample-points/"+id+"/maintainer").principal(()->ADMINISTRATOR).contentType(MediaType.APPLICATION_JSON)
    ...
    @Test void rejectsDisabledEmployeeAndOutsideRegion() throws Exception {
        mvc.perform(post(path()+"/preview").principal(()->ADMINISTRATOR).contentType(MediaType.APPLICATION_JSON).content("{\"regionCodes\":[\"230208\"]}"))
            .andExpect(status().isForbidden());
        jdbc.sql("UPDATE platform.security_user SET enabled=false WHERE subject_id=:subject").param("subject",subject).update();
        mvc.perform(post(path()+"/preview").principal(()->ADMINISTRATOR).contentType(MediaType.APPLICATION_JSON).content("{\"regionCodes\":[\""+REGION+"\"]}"))
    ...
    @Test void removingAllRegionsReleasesCurrentSamplesAndFutureSamples() throws Exception {
        ...
        var preview=mvc.perform(post(path()+"/preview").principal(()->ADMINISTRATOR).contentType(MediaType.APPLICATION_JSON)
    ...
    @Test void formerResponsibleEmployeeCannotUseLegacyBusinessWriteWithOverlappingAccess() throws Exception {
        ...
        var reader=transaction.execute(status->access.requireBusinessReadScope());
        assertThat(reader).isNotNull();
        assertThat(reader.isUnrestricted()).isTrue();
    }
    @Test void regionalAnnualReportingRemainsAdministratorOnly() throws Exception {
        ...
        annual(subject,county,0).andExpect(status().isForbidden());
        annual(ADMINISTRATOR,county,0).andExpect(status().isOk());
    }
    void handover(java.util.List<String> regions) throws Exception {
        String preview=mvc.perform(post(path()+"/preview").principal(()->ADMINISTRATOR).contentType(MediaType.APPLICATION_JSON)
    ...
    org.springframework.test.web.servlet.ResultActions save(String token) throws Exception {
        return mvc.perform(put(path()).principal(()->ADMINISTRATOR).contentType(MediaType.APPLICATION_JSON)
    ...
    String preview() throws Exception {
        return mvc.perform(post(path()+"/preview").principal(()->ADMINISTRATOR).contentType(MediaType.APPLICATION_JSON)
