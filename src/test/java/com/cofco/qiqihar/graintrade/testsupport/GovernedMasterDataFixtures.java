 ...
 * Their effective time is already in the past so clock jitter in emulated test databases
 * cannot turn an immediately applicable fixture into a scheduled change.
 */
public final class GovernedMasterDataFixtures {
    ...
    public static void insertRegion(
        ...
        jdbc.sql("""
                SELECT platform.govern_master_data_change(
                  ...
                  CURRENT_TIMESTAMP - interval '1 second','production-tester','market-tester',
                  '自动化测试双人复核地区主数据夹具')
    ...
    public static void deleteRegions(JdbcClient jdbc, Collection<String> codes) {
        ...
        governedCodes.forEach(code -> jdbc.sql("""
                SELECT platform.govern_master_data_change(
                  'REGION',region.code,'DELETE',to_jsonb(region),CURRENT_TIMESTAMP - interval '1 second',
